...
// const stateAll = require('../../HY/stateAll');
const sleep = require('../../HY/common/sleep');
const nodie = require('../engine_start');
// const PhotoCleansing = require('../../QR/Photo/PhotoCleansing');
const pLimit = require('p-limit');
const baseMethods = require('../baseMethods');
const limitS = pLimit(2);

/////////////////////图片清理//////time interval 有可能与电机命令冲突（如发送电机命令时，同时处理图片清理时间）///////
// const photoCleansingInterval = setInterval(() => {
//     PhotoCleansing.photoCleansing(-2);
// }, 12 * 3600 * 1000);//12 * 3600 *
const engineState = async () => {
    // const colis = await fetchStates.coils();
    // console.log(colis);
    // if (colis.coilsState.length > 0) {
    //     console.log('colis.coilsState.length > 0')
    //     if (!colis.coilsState[config.relay_rtu - 1]) {
    //         console.log('console.log(!colis.coilsState[config.relay_rtu - 1])', !colis.coilsState[config.relay_rtu - 1]);
    //         await motorOn();
    //         await sleep(config.rtu.engineNap);
    //         return true;
    //     } else {
    //         console.log(colis.coilsState[config.relay_rtu - 1]);
    //         return true;
    //     }
    //
    // } else {
    //     baseMethods.writeLog('疑是继电器未开启！');
    //     return false;
    // }
    // await motorOff();
    // await motorOn();
...
let obj = {
    ...
    async M_drawer_loop_Check(data, fn) {
        limitS(async () => {
                if (ws_imgEngine.shuo) {
                    await sleep(3000);
                    fn(1, 'waiting for last request to response', new Error());
